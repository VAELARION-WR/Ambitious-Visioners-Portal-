    ...
    <div className="max-w-md mx-auto px-4 py-10">
      <motion.div initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} className="rounded-2xl border p-6 bg-white shadow-sm">
        ...
        <form className="mt-6 space-y-4" onSubmit={onSubmit}>
          <input className="w-full border rounded-xl p-3" placeholder="Full name" value={name} onChange={e=>setName(e.target.value)} />
          ...
          <button disabled={loading} className="w-full rounded-xl bg-gray-900 text-white py-3">{loading?'Working...':'Sign up'}</button>
        </form>
        <p className="mt-3 text-sm text-gray-600">Already have an account? <Link to="/login" className="text-blue-700">Login</Link></p>
      </motion.div>
    </div>
  );
}
